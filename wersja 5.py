...
class PortScannerGUI:
    def __init__(self, window):
        ...
        self.window.configure(background="#f2f2f2")  # Ustawienie koloru tła
    ...
    def create_widgets(self):
        ...
        ttk.Label(ip_frame, text="Start IP:").pack(side=tk.LEFT)
        self.entry_start_ip = ttk.Entry(ip_frame)
        self.entry_start_ip.pack(side=tk.LEFT)

        ttk.Label(ip_frame, text="End IP:").pack(side=tk.LEFT, padx=(10, 0))
        ...
        ttk.Label(port_frame, text="End Port:").pack(side=tk.LEFT, padx=(10, 0))
        ...
        ttk.Label(timeout_frame, text="Timeout (ms):").pack(side=tk.LEFT)
        ...
        # Status bar
        self.status_bar = ttk.Label(self.window, text="IPs Scanned: 0%", anchor=tk.W, background="#f2f2f2")
        ...
        # Open ports counter
        self.open_ports_counter = ttk.Label(self.window, text="Open Ports: 0", background="#f2f2f2")
        self.open_ports_counter.pack(side=tk.BOTTOM, fill=tk.X)

        # Scanned ports counter
        self.scanned_ports_counter = ttk.Label(self.window, text="Scanned Ports: 0", background="#f2f2f2")
        self.scanned_ports_counter.pack(side=tk.BOTTOM, fill=tk.X)
    ...
    def start_scan(self):
        ...
        self.open_ports_count = 0
        self.scanned_ports_counter.configure(text="Scanned Ports: 0")
        self.scanned_ports_count = 0
    ...
    def scan_single_ip(self, ip, start_port, end_port, timeout):
        ...
        for port in range(start_port, end_port + 1):
            ...
            self.update_scanned_ports_counter()
    ...
    def update_status_bar(self, scanned_count, total_count):
        ...
        self.status_bar.configure(text=f"IPs Scanned: {percentage}%", background="#f2f2f2")
        self.progress_bar["value"] = scanned_count
        self.window.update()

    def update_open_ports_counter(self, count):
        self.open_ports_count += count
        self.open_ports_counter.configure(text=f"Open Ports: {self.open_ports_count}", background="#f2f2f2")

    def update_scanned_ports_counter(self):
        self.scanned_ports_counter.configure(text=f"Scanned Ports: {self.scanned_ports_count}", background="#f2f2f2")
